...
class ThemeManager {
    constructor(options) {
        ...
        //update current theme keys
        watch(() => this.theme.currentTheme, (newval, oldval) => {
            this.updateThemeTree(newval, oldval);

        });
        this.init(options);
        this.updateThemeTree(this.theme.currentTheme);
    }
    ...
    addTheme(style, themeName = 'default', activate = false) {
        if (activate) {
            ...
            try {
                this.onThemeChangedListener(this.theme.name, this.theme.currentTheme);
            } catch (err) {};
        }
    ...
    activateTheme(name) {
        if (name != '' || name != null) {
            if (this.theme[name] !== undefined) {
                ...
                this.saveTheme();
                //try{this.onThemeChangedListener(name,this.theme.currentTheme);}catch(err){};
    ...
    getTheme(themeName = this.theme.name, which) {
        ...
        for (let i = 0; i < which.length; i++) {
            let c = which[i]
            // console.log(JSON.stringify(this.theme['default']));
            try {
...
const VueThemeManager = {
    install(Vue, options) {
        Vue.config.globalProperties.$AppTheme = new ThemeManager(options);
        Vue.mixin({
            mounted() {
